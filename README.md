...
- **Hosting**: Render.com
- **Deployment**: `build.sh` + `render.yaml` *(демонстрационный)*
- **Fonts**: Zalando Sans (локально)
- **Icons**: SVG + PNG

- **OpenStreetMap + OpenCageData + Leaflet**: интерактивные карты

- Отправленные с сайта информации на почту (через форму заявки) - email-адреса Gmail с использованием App Passwords *(не render не работает)*;
...
## Сравнение реализацию функциии поиска: OpenStreetMap (с использованием Leaflet или Mapbox) и Google Maps:
...
Рекомендация для небольшого сайта:
- Если бюджет ограничен: OpenStreetMap + Mapbox (бесплатный тариф достаточно щедрый)
- Если важна простота внедрения: Google Maps (платный, но предсказуемый)
- Если нужны максимальная гибкость: OpenStreetMap + Leaflet (требует больше работы)
